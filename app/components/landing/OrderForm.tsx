...
// تخزين مؤقت بين إطلاق Pixel في المتصفح واستدعاء /api/sheet/order — يحمل
// eventId + Advanced Matching المُجزّأ كي يُرسل للـ CAPI server-side بدون
// إعادة حساب. يُمسح من window بعد الإرسال لتفادي التسرّب.
declare global {
  interface Window {
    __lastMetaEvent?: {
      eventId: string;
...
export function OrderForm({ product, preview = false }: { product: Product; preview?: boolean }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // eventId واحد يُرسل للـ fbq() وللـ CAPI server-side كي لا يُحسب
    // الـ Purchase مرتين (deduplication على مستوى Meta).
