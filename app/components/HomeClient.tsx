...
// محتوى الصفحة الرئيسية (عميل) — يفصله غلاف الخادم في `app/page.tsx` كي
// تُقرأ تجاوزات النصوص وقت البناء/التجديد وتُمرَّر بلا وميض.
export function HomeClient() {
  const { t } = useLocale();
  const [showAdmin, setShowAdmin] = useState(false);
  const [showGuest, setShowGuest] = useState(false);

  // فتح نافذة دخول الأدمن تلقائياً عند الوصول من /?admin=1 —
  // مسار الطرد من صفحة الإدارة (بلا جلسة صالحة) يوجّه هنا، فكانت
  // النافذة لا تُفتح ويظن المستخدم أن الزر لا يعمل.
  useEffect(() => {
    try {
        ...
        <div className="container-landing">
          <div className="liquid-glass liquid-glass--rounded grid gap-8 overflow-hidden rounded-[2rem] p-6 sm:p-10 lg:grid-cols-[1fr_0.9fr] lg:items-center">
            <div className="grid gap-4">
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                <a
                  ...
                  target="_blank"
                  rel="noopener"
                  className="inline-flex items-center gap-3 rounded-full bg-[#166fe5] px-8 py-4 font-bold text-white shadow-xl shadow-[#166fe5]/30 transition hover:bg-[#0f62c9] hover:shadow-2xl hover:shadow-[#0f62c9]/50"
                >
            ...
            <div className="relative">
              ...
              {/* eslint-disable-next-line @next/next/no-img-element -- صورة ثابتة محلية غير محسّنة عبر next/image (unoptimized) */}
              <img
                src="/fb.png"
