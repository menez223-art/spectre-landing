...
import { dzdToUsd } from "./utils/constants";
import {
  COMMUNES,
  DELIVERY_PRICES,
  DELIVERY_TYPES,
...
(function () {
  ...
  function applyItem(index) {
    ...
    // DZD غير مدعوم في Meta Pixel فنحوّله إلى USD عبر dzdToUsd().
  ...
  form.addEventListener("submit", function (ev) {
    ...
    // DZD غير مدعوم في Meta Pixel فنحوّل المبلغ إلى USD عبر dzdToUsd().
    ...
    if (PIXEL_ID && typeof fbq === "function") {
      try {
        ...
        // Advanced Matching في الـ HTML الاحتياطي: نُجزّأ phone فقط (لا نملك email
        // في الصفحة الاحتياطية). الـ Advanced Matching hash عبر SHA-256.
        // الصفة fn/ln تُحسب من الاسم المقسوم.
        ...
        if (typeof crypto !== "undefined" && crypto.subtle) {
          ...
          if (phoneDigits) hashedPhone = await sha(phoneDigits);
          if (firstName) hashedFirst = await sha(firstName.trim().toLowerCase());
          if (lastName) hashedLast = await sha(lastName.trim().toLowerCase());
        }
        fbq("track", "Lead", {
