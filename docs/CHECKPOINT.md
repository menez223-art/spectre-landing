...
## تحديث 2026-09-24 (٨) — مراجعة أمنية + E2E كامل (26/26) + مراجعة GitHub/Vercel قبل النشر (محلي — لم يُنشر شيء)

### أ) المراجعة الأمنية الشاملة (worktree d1986)
- `tsc` / `lint` / `build` نظيفة · 0 ملفات تالفة · لا أسرار مسرّبة في الكود.
- التعديلات الوحيدة: 4 ملفات (BOM ×3 + تعليقات توثيقية في `trial/*`) — **غير مُلتزمة**.

### ب) اختبار E2E كامل على خادم dev ببيانات الإنتاج — **26/26 ✅**
- الصفحات العامة الخمس 200 + صفحة 404 عربية سليمة لسلاگ غير موجود.
- API: رفض صحيح (401/403/400/429/503) · fail-closed لـ `/api/agent` بلا `AGENT_TRIAL_KEY` · لا تسريب في `/api/publish`.
- جلسة أدمن حقيقية: دخول + 10 مسارات قراءة + خروج + 403 بعده.
- فلو الاستوديو: دخول بجهاز جديد → رمز تحقق حقيقي أُرسل (Resend يعمل) · 401 timing-safe · can-produce/account/my-page-stats/factory-base/availability سليمة.
- ملفات الفحص المؤقتة (`e2e-check/`، `test-results/`) حُذفت بعد النجاح.

### ج) حادثة قفل دخول الأدمن (429) — حُلّت ووُثّقت
- **السبب:** مانع القوة الغاشمة (5 محاولات/15د لكل IP) — اختبارات E2E استهلكت محاولتين ومحاولات المالك اليدوية أكملت السقف على عنوان `::1` (localhost). البيانات نفسها كانت صحيحة دائماً.
- **الحل:** حذف مفاتيح `ratelimit/admin-login/*` من جدول `kv` في Supabase مباشرة → دخول 200 فوري.
- **ملاحظة:** رُصدت محاولتان خارجيتان (نطاق 105.235.x.x جزائري) على الإنتاج — ضجيج إنترنت طبيعي، الحماية صدّتهما.

### د) مراجعة GitHub (`menez223-art/spectre-landing` — **PUBLIC**)
- آخر push: **2026-09-21** · الفرع الافتراضي `main` · آخر commit `d74f167`.
- **فحص الأسرار:** شجرة الملفات كاملة (203 ملفاً) نظيفة — لا `.env` ولا مفاتيح ولا أسرار (المطابقات الخمس «credentials» ملفات كود مشروعة لإدارة بيانات الدخول).
- ⚠️ **انحراف التاريخ:** `master` المحلي (`54f3f6a`) **commit جذري بلا أب** — لا يشترك بأي تاريخ مع `main` على GitHub. الفرق بالمحتوى ضئيل (6 ملفات: LocaleProvider/layout/storage/trialStore/date + `.freebuff/project-id`) لكن **الدفع المباشر سيتطلب force-push أو إعادة ربط يدوية**.

### هـ) مراجعة Vercel (`spectre` — حساب menez223-7187)
- المشروع مربوط CLI (`.vercel/project.json`) · Node 24.x · إطار Next.js مكتشف تلقائياً.
- **متغيرات الإنتاج (18): كلها موجودة وسليمة** — `ADMIN_EMAIL` تحقّقنا من قيمته الفعلية عبر pull مؤقت (بريد صحيح بطول 18 حرفاً؛ عرض `eyJ2...` في `env ls` كان تنسيق عرض CLI لا خللاً). الملف المؤقت حُذف فوراً.
- **الإنتاج حيّ:** `spectre-dz.vercel.app` = 200 — لكن HTML يُظهر `<html lang="ar" dir="rtl">` **بدون** `suppressHydrationWarning` ⇒ الإنتاج يعمل بنسخة **قبل** commit `54f3f6a` (إصلاحات viewport/locale/date/slugify/trial-audit غير منشورة بعد).
- Cron مضبوط: `/api/admin/link-health?action=auto` يومياً 03:00.

### و) الحالة النهائية
- **لا commit ولا push ولا نشر** — كل شيء محلي بانتظار أمر المالك.
- الخادم المحلي `localhost:3000` متروك يعمل بطلب المالك.
- **قبل النشر القادم يُنصح بـ:** (1) حسم انحراف تاريخ git (force-push مقابل rebase) · (2) إضافة `.freebuff/` إلى `.gitignore`/`.vercelignore` · (3) حسم مصير التعديلات الأربعة غير المُلتزمة.
...
### 2026-09-25: تحسين شامل للواجهة قبل النشر وتصحيح التباين وفصل التخزين
1. **فصل تعارض التخزين المحلي لـ `spectre-theme`**:
   - `ThemeSelector.tsx` الآن يستخدم `spectre-accent-theme` حصرياً لتخزين نغمة اللون (Default, Blue, Purple, Rose, Orange, Cyan)، بينما احتفظ `ThemeProvider.tsx` بمفتاح `spectre-theme` لوضع الإضاءة (`light` / `dark` / `system`).
   - تم تحديث `app/theme-script.ts` ليقرأ ويطبّق السمتين معاً قبل رسم الصفحة (`.dark` و `data-theme`) لمنع أي وميض تحميل FOUC لكلا الخاصيتين.

2. **إصلاح قص القائمة المنسدلة (Menu Clipping)**:
   - إزالة `overflow-hidden` من حاوية الزجاج السائل `liquid-glass--pill` في `PageHeader.tsx` مما أتاح لقائمة اختيار الألوان التمدد والتفاعل السلس دون أن تُقتطع خلف المحتوى الرئيسي (تم التحقق بـ Hit Test واختبار النقر بمتصفح Playwright).

3. **تصحيح التباين والامتثال لمعايير WCAG AA**:
   - رفع تباين نصوص الأزرار الحاثة على الإجراء (CTAs): تحويل الأزرار من التدرجات الفاتحة غير المتوافقة إلى `theme-gradient` و `#047857` / `#0f766e` مع تباين يتجاوز `5.48:1` (فوق حد 4.5:1 المطلوب).
   - تحسين النصوص الإرشادية (Placeholders): استبدال العتامة الضعيفة `30%` و `35%` بنسبة `60%` في الحقول، مما رفع التباين من `2.22:1` (راسب) إلى `4.67:1` (ناجح AA).
   - تحسين نصوص التذييل `PageFooter.tsx`: رفع التباين من `3.33:1` إلى `6.35:1` في الوضع النهاري و `9.34:1` في الوضع الليلي.
   - تحسين شارات أسعار الخطط في `app/pricing/page.tsx` لتعتمد ألوان الثيم الموحدة وتدرجات عالية القراءة.

4. **التحقق التقني**:
   - الفحص الساكن: `npx tsc --noEmit` نظيف بدون أي أخطاء من نوع TypeScript.
   - سكريبت التباين `ui-audit/contrast.mjs`: جميع العناصر أصبحت تحقق علامة `AA✓`.

### 2026-09-25 (تكملة): النشر على GitHub و Vercel + تدقيق ما بعد النشر وإغلاق بقايا التباين

**أ) النشر:**
- حُسم انحراف تاريخ git بالدمج لا بالكتابة فوق التاريخ: أُنشئ فرع `main` محلي من
  `origin/main` ثم `git merge --allow-unrelated-histories -X theirs cline/d1986`
  — حُفظ تاريخ الريموت كاملاً وصار شجرة الناتج مطابقة تماماً لفرع العمل.
- `main` مدفوع إلى https://github.com/menez223-art/spectre-landing (HEAD = `f9a4f10`).
- نشر الإنتاج عبر Vercel CLI على مشروع `spectre` (عيّنة `menez223-7187s-projects`):
  `https://spectre-dz.vercel.app` — البناء نجح في 1m و 157 ملفاً بحجم رفع 905KB.

**ب) تدقيق ما بعد النشر** (`ui-audit/prod-verify.mjs`): يقيس حالة كل صفحة، التمرير
الأفقي (موبايل/ديسكتوب)، عدد وسوم viewport، أخطاء التشغيل، قائمة الألوان بالنقر
الفعلي (Hit-Test)، وأسوأ نسب تباين نصية محسوبة من الأنماط المركّبة فعلياً في المتصفح.
النتيجة الأولى: **41/50**.

**ج) ما أُصلح بعد التدقيق:**
1. زر فيسبوك (الرئيسية): أبيض على `#1877f2` = 4.23:1 → `#166fe5` (4.73:1) وتحويم `#0f62c9` (5.82:1).
2. بطاقة المنتج: `text-navy-900/45` (2.89:1) و `text-navy-900/40` (2.54:1) → `/65` في الوضعين (5.3 – 7.5:1).
3. عدّاد القسم في المتجر `text-navy-900/45 dark:text-ivory-50/45` → `/65`.
4. شارة المالك «بواسطة»: `text-emerald-600` على بطاقة بيضاء (3.77:1) → `text-emerald-700` (5.48:1).
5. `globals.css`: إضافة تجاوز `.dark .text-navy-900\/40` (0.62) ورفع `/45` إلى 0.65.
6. تصحيح دالة قياس التباين نفسها: كانت تتجاهل شفافية طبقات الخلفية الوسيطة فأبلغت
   زوراً 1.05:1 عن شارة عدّاد فوق طبقتين شبه شفافتين (التركيب الآن من الخارج للداخل
   فوق أساس معتم).
7. إزالة وسم `<meta name="viewport">` اليدوي من `app/layout.tsx` — Next.js يحقنه
   تلقائياً وكان الوسم يتكرر مرتين في HTML المُقدَّم.
8. استثناء `ui-audit/` و `.freebuff/` من `.gitignore` و `.vercelignore` (أدوات/بقايا محلية).

**د) النتيجة النهائية: 50/50 PASS** — أولاً على `localhost:3000` ثم بعد إعادة النشر
على الإنتاج: صفر إخفاق تباين في (home · pricing · store) × (نهاري · ليلي) ×
(ديسكتوب 1440 · موبايل 390)، صفر تمرير أفقي، وسم viewport واحد، صفر أخطاء تشغيل،
وكل عناصر قائمة الألوان الستّة قابلة للنقر بلا قص، مع حفظ النغمة في
`spectre-accent-theme` والوضع في `spectre-theme` بشكل مستقل والبقاء ثابتين بلا وميض
بعد إعادة التحميل.

### 2026-09-25 (خِتام): تنظيف الكاش + التحقق ببناء بارد
- أوقف خادم التطوير (PID المستمع على 3000) وحُذفت مخرجات وكاشات البناء: `.next`
  (**213.56 MB**)، و`out`، و`node_modules/.cache`، و`*.tsbuildinfo`، و`.turbo`،
  وكل ملفات `.tmp-*` المؤقتة. تأكيد: لا شيء من هذه المسارات موجود بعد التنظيف.
- **تحقق ببناء بارد من الصفر** بعد الحذف: `npm run build` → `✓ Compiled successfully`
  و10/10 صفحات ثابتة و`Collecting build traces` بلا أي خطأ (69 سطر سجل، صفر matches
  لأنماط `Failed to compile|Error:|error TS|ELIFECYCLE`). ثم حُذفت مخرجات البناء
  لإبقاء مساحة العمل نظيفة.
- أُعيد تشغيل الخادم المحلي على http://localhost:3000 (استجابة 200) بحالة نظيفة.
- **الحالة النهائية للحفظ:** `main` = `cline/d186` = `5890ba8` على GitHub، والإنتاج
  https://spectre-dz.vercel.app (نشر `spectre-33w21hna8`، Ready في 46s).
- **ملاحظات تشغيلية مفتوحة (لم تُنفَّذ بعد):**
  1. مشروع Vercel غير مربوط بـ Git Integration — دفع `main` لا يطلق نشراً تلقائياً،
     فالنشر يتم بـ `npx vercel --prod` من مجلد العمل.
  2. `<html>` يبقى بلا `lang`/`dir` في HTML الأولي (يُضبَطان بعد الترطيب عبر
     `LocaleProvider`) — الأثر: SEO أضعف ووميض اتجاه محتمل لعائد بالعربية.
     الإصلاح المقترح: توسيع `themeNoFlashScript` ليضبط `html.lang`/`html.dir` من
     `landing-studio-lang` قبل أول رسم، أو تمرير اللغة من السيرفر.
  3. لم يُجرَّب تدقيق تباين على لوحات الإدارة/الاستوديو بعد تسجيل الدخول (خارج
     الصفحات العامة) — يمكن توسيع `ui-audit/prod-verify.mjs` بجلسة دخول لتغطيتها.
