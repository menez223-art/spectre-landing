...
| Meta Pixel | ✓ | ✓ | ✓ |
| وضع المتجر (متعدد) | ✗ | ✓ | ✓ |
- التاغ لاين تحت السعر: Basic = "منتج واحد مع صورتان"، Pro = "حتى 5 منتجات مع 5 صور"، Gold = "حتى 10 منتجات مع 10 صور" (بطلب المستخدم صرلم تُلمس** صفحة الهبوط المنتجة `app/p/[slug]/page.tsx` و `app/components/landing/*` (بطلب المستخدم صراحة).
- **لم يُغيَّر** `authStore` / `isDeviceBanned` / قائمة الأجهزة.
- **لم تُكشف** أسرار `.env.*`.
- **لم يُنش إذن صريح. التوصية: (1) `vercel deploy` كـ preview أولاً (2) اختبار سريع `/`, `/studio`, `/pricing` على الـ preview (3) ثم `vercel --prod` للترقية.
