 ...
 *      (بترويسة الأعمدة التسعة) ومشاركته مع البريد.
 *   2) تسجيل طلبات كل مستخدم في جدوله عبر ?key=...
 *   3) عمود "تاريخ ووقت" يُملأ تلقائيًا عند كل إدخال جديد.
 ************************************************************/

var SECRET = "664a8e25d3cd0c916c023dec821d39830112daa3fdfd9c779f7c018dcd9c78fc";

// نقطة الإعلان على خادمنا (Vercel) — نُعلن إليها رابطنا الحيّ الحالي بعد كل
// استدعاء، فتبقى FACTORY_URL محدّثة تلقائياً. اضبطها على رابط:
//   https://<نطاقك>/api/sheet/announce
// واتركها فارغة لتعطيل هذه الميزة (سيُستخدم متغيّر البيئة FACTORY_URL فقط).
...
function doGet(e) {
  announceSelf(); // نُعلن رابطنا الحالي عند أي فحص صحة/فتح — فيتعافى FACTORY_URL تلقائياً
...
function doPost(e) {
  try {
    ...
    // نبضة قلب: نُعلن رابطنا الحيّ الحالي لخادمنا في كل استدعاء (دون انتظار الرد)
    announceSelf();
...
// يردّ دائماً الرابط الحالي لـ Apps Script عبر getService().getUrl()
// (يتغيّر تلقائياً بعد كل "نشر جديد")، فلا حاجة لحفظ الرابط أو لصقه يدوياً.
function lookupSheet(data) {
  ...
  // الرابط الحيّ الحالي — يُعاد دائماً من دالة getService() (يعكس أحدث نشر)
...
// نبضة قلب: نُعلن رابطنا الحيّ الحالي (بعد أي "نشر جديد") لخادمنا في Vercel
// حتى يبقى FACTORY_URL محدّثاً تلقائياً دون تدخل يدوي. نُرسل بشكل غير متزامن
// (نطلق الطلب ولا ننتظره) كي لا نبطئ تسجيل الطلبات، ونخفق بهدوء عند أي خطأ.
